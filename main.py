...
from util import stereographicProjection, cylindricalProjection, shiftedCylindricalProjection
...
def main(config_file: str, output_file: str | None):
    ...
    projection = shiftedCylindricalProjection
    ...
    bright, = np.where(np.logical_and(
        ...
        abs(star_centers[:, 0]) - 2*star_markers <= display.width / 2,
        abs(star_centers[:, 1]) - 2*star_markers <= display.height / 2,
    ))
    ...
    output_name = output_file or config.output.name
    dwg = svg.Drawing(
        filename=str(PurePath(output_name).with_suffix(".svg")),
        size=page.size
    )
    dwg.viewbox(
    ...
    for center, marker in zip(star_centers[bright], star_markers[bright]):
        ...
        # Clips over right edge
        if center[0] + 2*marker > display.width / 2:
            fakeCenter = center - [display.width, 0]
            star_group.add(Circle(center=fakeCenter, r=marker))

        # Clips over left edge
        elif center[0] - 2*marker < -display.width / 2:
            fakeCenter = center + [display.width, 0]
            star_group.add(Circle(center=fakeCenter, r=marker))

        if center[1] + 2*marker > display.height / 2:
            fakeCenter = [center[0] % display.width - display.width / 2, display.height - center[1]]
            star_group.add(Circle(center=fakeCenter, r=marker))

        elif center[1] - 2*marker < -display.height / 2:
            fakeCenter = [center[0] % display.width - display.width / 2, -display.height - center[1]]
            star_group.add(Circle(center=fakeCenter, r=marker))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="Star Cover")
    parser.add_argument('-c', "--config", default="config.toml")
    parser.add_argument('-o', "--output")
    args = parser.parse_args()
    main(config_file=args.config, output_file=args.output)
